...
func (i *InvalidSequenceNumbers) Report(encoder *json.Encoder) error {
	return encoder.Encode(i.Terminals)
}

func (i *InvalidSequenceNumbers) Fix() iter.Seq[any] {
	shouldUpgrade := make(map[uuid.UUID]struct{})
	for k, terminals := range i.Terminals {
		for j := range terminals {
			seq := terminals[j].SequenceNumber
			newSeq := j + 1
			if seq != newSeq {
				terminals[j].SequenceNumber = newSeq
				i.Terminals[k] = terminals
				shouldUpgrade[terminals[j].Mrid] = struct{}{}
			}
		}
	}

	return func(yield func(v any) bool) {
		for _, terminals := range i.Terminals {
			for _, term := range terminals {
				if _, ok := shouldUpgrade[term.Mrid]; !ok {
					continue
				}
				if !yield(term) {
					return
